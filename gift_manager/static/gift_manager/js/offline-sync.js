...
class OfflineSyncManager {
    ...
    async updateConnectionStatus() {
        ...
        if (this.isOnline) {
            ...
            statusEl.innerHTML =
                '<i class="fas fa-wifi-slash text-danger"></i> <span class="d-none d-sm-inline">Offline</span>';
    ...
    async triggerSync() {
        ...
        if ("serviceWorker" in navigator && "sync" in navigator.serviceWorker) {
            try {
                ...
                console.warn(
                    "[Offline Sync] Background sync not available, falling back to immediate sync"
                );
            }
        }
        ...
        for (const item of pending) {
            try {
                ...
                if (item.type === "create" && result && item.context && item.context.sourceUrl) {
                    ...
                    if (
                        window.location.href === item.context.sourceUrl ||
                        window.location.href.split("#")[0] === item.context.sourceUrl.split("#")[0]
                    ) {
    ...
    async processSyncItem(item) {
        ...
        if (type === "create") {
            ...
            if (serverData.data) {
                ...
                try {
                    ...
                    if (!verifyEntry) {
                        console.error(
                            "[Offline Sync] ✗ CRITICAL: Entry not found after atomic swap!"
                        );
                    }
                } catch (error) {
        ...
        if (type === "update") {
            ...
            try {
                ...
                console.warn(
                    "[Offline Sync] Failed to update entry in IndexedDB after update sync:",
                    err
                );
            }
        }
    ...
    async handleFormSubmit(event, model, type, objectId = null) {
        ...
        this.showToast(
            ...
            } ${type}d offline - will sync when reconnected`,
            "info"
        );
    ...
    async triggerLogoutCleanup() {
        try {
            ...
            if (window.giftManagerDB) {
                ...
                const storesToClear = ["gifts", "persons", "events", "relations", "groups", "tags"];
