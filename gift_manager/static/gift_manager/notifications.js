...
function showNotification(message, type = 'success', duration = null, options = {}) {
  ...
  if (duration === null) {
    ...
    switch(type) {
      case 'success': duration = Math.min(baseTime, 5000); break;  // 3-5 seconds for successes
      case 'info': duration = Math.min(baseTime, 6000); break;     // 3-6 seconds for info
      case 'warning': duration = Math.min(baseTime, 8000); break;  // 3-8 seconds for warnings
      case 'danger': duration = Math.min(baseTime, 12000); break;  // 3-12 seconds for errors
  ...
  document.body.addEventListener('htmx:afterRequest', function(e) {
    ...
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      if (successMessage) {
        ...
        const method = (element.getAttribute('hx-post') ? 'POST' :
