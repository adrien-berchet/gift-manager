...
class TestBulkOperationsProperty:
    ...
    def test_bulk_operations_support_property(self, entity_type, entity_count):
        ...
        **Validates: Requirements 6.1, 6.2, 6.3, 6.4**
        ...
        assert 'modal-header' in confirmation_content, \
            "Confirmation should display as modal"
        ...
        assert total_processed > 0, \
            "Bulk delete should process at least some items"
        ...
        assert 'message' in response_data, \
            "Bulk delete should provide success message"
    ...
    def test_bulk_share_support_property(self, entity_type, entity_count):
        ...
        **Validates: Requirements 6.1, 6.2**
        ...
        assert bulk_response.status_code == 200, \
            f"Bulk share should be available for {entity_type}"
        ...
        assert 'redirect_url' in response_data, \
            "Bulk share should provide redirect URL"

        redirect_url = response_data['redirect_url']
        assert '/share/' in redirect_url, \
            "Redirect URL should point to share page"
        assert entity_type in redirect_url, \
            "Redirect URL should include entity type"
    ...
    @given(
        invalid_action=st.text().filter(lambda x: x not in ['bulk_delete', 'bulk_share'])
    )
    def test_bulk_operations_invalid_action_property(self, invalid_action):
        ...
        **Validates: Requirements 6.3, 6.4**
        ...
        assert bulk_response.status_code == 400, \
            "Invalid bulk action should return 400 status"

        response_data = json.loads(bulk_response.content)
        assert response_data.get('success') is False, \
            "Invalid bulk action should not succeed"
        assert 'error' in response_data, \
            "Invalid bulk action should provide error message"
    ...
    def test_bulk_operations_empty_selection_property(self, entity_type):
        ...
        **Validates: Requirements 6.1, 6.2**
        ...
        assert bulk_response.status_code == 400, \
            "Empty selection should return 400 status"

        response_data = json.loads(bulk_response.content)
        assert response_data.get('success') is False, \
            "Empty selection should not succeed"
        assert 'error' in response_data, \
            "Empty selection should provide error message"
