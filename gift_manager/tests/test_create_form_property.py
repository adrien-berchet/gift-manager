...
class TestCreateFormProperty:
    ...
    def test_successful_operation_completion_create(self, entity_type, entity_data):
        ...
        **Validates: Requirements 5.3**
        ...
        if is_error_response:
            ...
            assert 'error' in hx_trigger.lower(), f"Error response should have error notification for {entity_type}"
        ...
        if entity_type.lower() == 'person':
            ...
            if form_data.get('email_address'):
                ...
                assert created_entity.email_address is not None, \
                    f"Person email_address not set"
        ...
        if response.status_code == 200:
            ...
            if not is_error_response:
                ...
                if has_errors:
                    ...
                    actual_errors = any(re.search(pattern, content, re.IGNORECASE) for pattern in error_patterns)
                    assert not actual_errors, f"Response contains error messages for {entity_type}: {content[:500]}"
        ...
        if fallback_response.status_code == 302:
            ...
            url_patterns = [
                entity_type.lower(), 'list', 'detail', 'person_groups', 'gift_tags', 'gift-tag'
            ]
    ...
    def test_ui_component_display_consistency_create(self, entity_type, entity_data):
        ...
        **Validates: Requirements 5.1**
        ...
        assert fallback_response.status_code == 200, f"Fallback create page failed for {entity_type}"
        ...
        type_found = (entity_type_display in fallback_content or
                     ...
                     'create' in fallback_content.lower())
