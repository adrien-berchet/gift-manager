...
class TestCreateFormValidationProperty:
    ...
    def get_model_class(self, entity_type):
        ...
        from gift_manager.models import Person, Gift, Event, Relation, PersonGroup, GiftTag
    ...
    @given(
        ...
        invalid_data=st.sampled_from(['empty_required', 'invalid_email', 'too_long', 'missing_relations', 'invalid_relations'])
    )
    ...
    def test_error_handling_consistency_create(self, entity_type, invalid_data):
        ...
        **Validates: Requirements 5.4**
        ...
        if entity_type.lower() == 'person' and invalid_data in ['missing_relations', 'invalid_relations']:
            return  # Skip invalid combinations
        if entity_type.lower() in ['gift', 'event', 'persongroup', 'gifttag'] and invalid_data == 'invalid_email':
            return  # Skip invalid combinations
        if entity_type.lower() != 'relation' and invalid_data in ['missing_relations', 'invalid_relations']:
            return  # Skip invalid combinations
        ...
        if entity_type.lower() == 'relation' and invalid_data not in ['missing_relations', 'invalid_relations']:
        ...
        has_error_content = any(error_indicator in content.lower()
        ...
        if invalid_data == 'empty_required':
            ...
            has_required_error = any(indicator in content.lower() for indicator in required_indicators)
            assert has_required_error, \
                f"Required field error message missing for {entity_type}"
            ...
            assert has_email_error, \
                f"Email validation error message missing for {entity_type}"
            ...
            assert has_length_error, \
                f"Length validation error message missing for {entity_type}"
            ...
            has_relation_error = any(indicator in content.lower() for indicator in relation_indicators)
        ...
        if response.status_code == 200:
            ...
            assert has_error_styling, \
                f"Error styling missing in response for {entity_type}"
            ...
            assert has_form_elements, \
                f"Form elements missing in error response for {entity_type}"
        ...
        if fallback_response.status_code == 200:
            ...
            assert 'form' in fallback_content.lower(), \
                f"Fallback form missing for {entity_type}"

            # Should contain error indicators
            fallback_has_errors = any(error_indicator in fallback_content.lower()
                                    for error_indicator in ['error', 'invalid', 'required', 'danger'])
            assert fallback_has_errors, \
                f"Fallback error indicators missing for {entity_type}"
        ...
        if response.status_code == 200:
            ...
            assert has_submit_button, \
                f"Submit button missing in error response for {entity_type}"

        # Verify that the error response doesn't contain full page structure (for HTMX)
        if response.status_code == 200 and 'HX-Request' in self.client.defaults.get('HTTP_HX_REQUEST', ''):
        ...
        if response.status_code == 200:
            ...
            error_patterns = [
                'alert-danger', 'invalid-feedback', 'error', 'required', 'invalid'
            ]
