...
class TestDetailViewDisplayProperty:
    ...
    def test_data_population_accuracy_detail(self, entity_type, entity_data):
        ...
        **Validates: Requirements 3.2, 3.3**
        ...
        assert htmx_response.status_code == 200, f"HTMX detail view failed to load for {entity_type}"

        htmx_content = htmx_response.content.decode()

        # HTMX response should contain detail content
        assert 'detail' in htmx_content.lower() or 'group details' in htmx_content.lower() or 'person details' in htmx_content.lower() or 'gift details' in htmx_content.lower() or 'event details' in htmx_content.lower() or 'relation details' in htmx_content.lower() or 'tag details' in htmx_content.lower(), f"HTMX response missing detail content for {entity_type}"
        ...
        if entity_str.strip():
            ...
            has_entity_data = (entity_str in htmx_content or
                             escaped_entity_str in htmx_content or
                             any(str(getattr(entity, field, '')) in htmx_content
                                 for field in expected_fields
                                 if hasattr(entity, field) and getattr(entity, field)))

            assert has_entity_data, \
                f"Entity data not found in HTMX detail view for {entity_type}"

        # Verify detail sections are properly structured
        detail_structure_elements = ['detail-section', 'detail-field', 'detail-value', 'detail-label']
        ...
        assert has_action_buttons, \
            f"Action buttons missing in detail view for {entity_type}"
        ...
        assert has_accessibility, \
            f"Accessibility attributes missing for {entity_type}"
    ...
    def test_ui_component_display_consistency_detail(self, entity_type, entity_data):
        ...
        **Validates: Requirements 3.1**
        ...
        detail_indicators = [
            'detail', 'group details', 'person details', 'gift details',
            'event details', 'relation details', 'tag details',
            'detail-section', 'detail-field', 'detail-value',
            f'{entity_type.lower()} details', 'creation date', 'created'
        ]
        ...
        if entity_str.strip():
            import html
            escaped_entity_str = html.escape(entity_str)
            has_entity_info = (entity_str in content or
                             escaped_entity_str in content or
                             entity_type.lower() in content.lower())
            assert has_entity_info, \
                f"Entity information missing for {entity_type}"
        ...
        assert has_bootstrap_classes, \
            f"Bootstrap detail classes missing for {entity_type}"
        ...
        assert has_action_elements, \
            f"Action elements missing for {entity_type}"
        ...
        assert has_icons, \
            f"Icons missing for {entity_type}"

        # Test that non-HTMX requests also work (fallback behavior)
        fallback_response = self.client.get(detail_url)
        assert fallback_response.status_code == 200, f"Fallback detail page failed for {entity_type}"
        ...
        has_detail_content = any(indicator in fallback_content.lower() for indicator in detail_content_indicators)
        ...
        if hasattr(entity, 'creation_date') and entity.creation_date:
            date_found = any(date_format in content for date_format in [
