...
class TestEditFormDisplayProperty:
        ...
        **Validates: Requirements 2.2**
        ...
        for field_name in expected_fields:
            if field_name in form.fields:
                ...
                assert field_id in content, f"Field {field_name} not rendered in HTML for {entity_type}"
        ...
        assert 'form' in htmx_content.lower(), f"HTMX response missing form elements for {entity_type}"
        ...
        assert 'csrfmiddlewaretoken' in htmx_content, f"CSRF token missing in HTMX response for {entity_type}"
        ...
        if entity_str.strip():  # Only check if entity has a meaningful string representation
            ...
            assert has_entity_data, \
                f"Entity data not found in HTMX form for {entity_type}"

        # Verify form action points to correct URL
        assert edit_url in htmx_content, f"Form action URL not found in HTMX response for {entity_type}"

        # Test that form contains proper Bootstrap classes for styling
        bootstrap_classes = ['form-control', 'form-select', 'form-input-text', 'form-textarea', 'form-check-input']
        ...
        if hasattr(form, 'is_valid'):
            ...
            if not validation_form.is_valid():
                ...
                non_required_errors = {
                    field: errors for field, errors in validation_form.errors.items()
                ...
                if non_required_errors:
                    assert False, f"Form validation failed for {entity_type} with entity's own data: {non_required_errors}"
        ...
        assert has_error_handling, \
            f"Error handling structure missing for {entity_type}"
    ...
    def test_ui_component_display_consistency_edit(self, entity_type, entity_data):
        ...
        **Validates: Requirements 2.1**
        ...
        assert has_bootstrap_classes, \
            f"Bootstrap form classes missing for {entity_type}"
        ...
        type_found = (entity_type_display in fallback_content or
                     entity_type.lower() in fallback_content.lower() or
                     'edit' in fallback_content.lower())
