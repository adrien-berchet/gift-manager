...
class TestInlineEditingProperty:
    ...
    def test_inline_editing_functionality_property(self, field_name, new_value):
        ...
        **Validates: Requirements 4.3, 4.4**
        ...
        cleaned_new_value = "".join(
            char for char in str(new_value) if ord(char) >= 32 or char in "\t\n\r"
        )
        ...
        assert response_data["success"] is True, (
            f"Update failed: {response_data.get('error', 'Unknown error')}"
        )
        ...
        assert response_data["new_value"] == cleaned_new_value, (
            "New value mismatch (should be cleaned)"
        )

        # Property: Database should be updated
        person.refresh_from_db()
        updated_value = getattr(person, field_name)
        assert updated_value == cleaned_new_value, (
            f"Database not updated: expected {cleaned_new_value}, got {updated_value}"
        )
    ...
    def test_inline_editing_permission_property(self, field_name, permission_level):
        ...
        **Validates: Requirements 4.3, 4.4**
        ...
        # Make inline update request
        url = reverse("gift_manager:person_inline_update", kwargs={"pk": person.person_id})
        data = {"field": field_name, "value": "New Value"}
    ...
    @given(
        invalid_field=st.text().filter(
            lambda x: x not in ["first_name", "family_name", "email_address"]
        )
    )
    def test_inline_editing_invalid_field_property(self, invalid_field):
        ...
        **Validates: Requirements 4.3, 4.4**
        ...
        url = reverse("gift_manager:person_inline_update", kwargs={"pk": person.person_id})
        data = {"field": invalid_field, "value": "Some Value"}
        ...
        assert "not editable inline" in response_data["error"], (
            "Error message should mention field not editable"
        )
