...
class KeyboardAccessibilityPropertyTest(TestCase):
    ...
    def test_modal_focus_management_property(self, entity_type):
        ...
        try:
            ...
            response = self.client.get(
                reverse(delete_url, kwargs={"pk": entity_pk}),
    ...
    def test_offcanvas_focus_management_property(self, entity_type):
        ...
        try:
            ...
            response = self.client.get(
                reverse(edit_url, kwargs={"pk": entity_pk}),
                HTTP_HX_REQUEST="true"
            )
        except Exception:
            # Skip if URL doesn't exist or other issues
    ...
    def test_keyboard_shortcuts_support_property(self, entity_type):
        ...
        assert 'data-keyboard-hint="Ctrl+S"' in content, f"Save button should have Ctrl+S hint for {entity_type}"
        assert 'title=' in content and 'Ctrl+S' in content, f"Save button should have Ctrl+S in title for {entity_type}"
    ...
    def test_aria_labels_and_screen_reader_support_property(self, entity_type):
        ...
        assert 'role="status"' in content, f"Loading state should have status role for {entity_type}"
        assert 'aria-live="polite"' in content, f"Loading state should have aria-live for {entity_type}"
        ...
        assert ('alert' in content.lower() or 'form' in content.lower()), f"Form or error handling should be present for {entity_type}"
    ...
    @given(
        shortcut_key=st.sampled_from(['Escape', 'Enter', 'Tab'])
    )
