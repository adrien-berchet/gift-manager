...
from gift_manager.tests.factories import (
    UserFactory, PersonFactory, GiftFactory, EventFactory,
    RelationFactory, PersonGroupFactory, GiftTagFactory
)
...
class TestRealTimeListFeaturesProperty:
        ...
        for i in range(entity_count):
            if entity_type == 'person':
                entity = PersonFactory(
                    first_name=f"Test{search_term}{i}",
                ...
                person = PersonFactory(
                    first_name=f"Person{search_term}{i}",
                    user_link=self.user
                )
                gift = GiftFactory(
                    name=f"Gift{i}",
                ...
                entity = PersonGroupFactory(
                    name=f"Group{search_term}{i}",
                    shared_with=[self.user]
                )
                entities.append(entity)
            elif entity_type == 'gifttag':
                entity = GiftTagFactory(
                    name=f"Tag{search_term}{i}",
                    shared_with=[self.user]
                )
                entities.append(entity)
    ...
    def test_real_time_search_response_structure_property(self, entity_type):
        ...
        **Validates: Requirements 7.2**
        ...
        if entity_type == 'person':
            ...
            expected_fields = ['person_id', 'first_name', 'family_name', 'email_address', 'groups_info']
        elif entity_type == 'gift':
            entity = GiftFactory(
                name="TestGift",
                shared_with=[self.user]
            ...
            expected_fields = ['event_id', 'name', 'comment', 'usual_date', 'absolute_date', 'recurrence']
        elif entity_type == 'relation':
            person = PersonFactory(user_link=self.user)
            gift = GiftFactory(shared_with=[self.user])
            entity = RelationFactory(
    ...
    def test_real_time_search_permission_filtering_property(self, entity_type):
        ...
        **Validates: Requirements 7.2**
        ...
        if entity_type == 'person':
            my_entity = PersonFactory(
                first_name="MyPerson",
                user_link=self.user
            )
            other_entity = PersonFactory(
                first_name="OtherPerson",
                user_link=other_user
            )
        elif entity_type == 'gift':
            my_entity = GiftFactory(
                name="MyGift",
                shared_with=[self.user]
            )
            other_entity = GiftFactory(
                name="OtherGift",
                shared_with=[other_user]
            )
        elif entity_type == 'event':
            my_entity = EventFactory(
                name="MyEvent",
                shared_with=[self.user]
            )
            other_entity = EventFactory(
                name="OtherEvent",
                shared_with=[other_user]
            )
        elif entity_type == 'relation':
            ...
            my_entity = PersonGroupFactory(
                name="MyGroup",
                shared_with=[self.user]
            )
            other_entity = PersonGroupFactory(
                name="OtherGroup",
                shared_with=[other_user]
            )
        elif entity_type == 'gifttag':
            my_entity = GiftTagFactory(
                name="MyTag",
                shared_with=[self.user]
            )
            other_entity = GiftTagFactory(
                name="OtherTag",
                shared_with=[other_user]
            )
        ...
        if data['data']:
            ...
            assert not other_entity_found, f"Other user's {entity_type} should not be found in search results"
