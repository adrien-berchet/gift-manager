...
class TestUnsavedChangesProtectionProperty:
    ...
    def create_test_entities(self, entity_type, count=3):
        ...
        for i in range(count):
            if entity_type == "person":
                ...
                entity = Gift.objects.create(name=f"Test Gift {i}", comment=f"Test comment {i}")
            elif entity_type == "event":
                entity = Event.objects.create(name=f"Test Event {i}", comment=f"Test comment {i}")
            elif entity_type == "persongroup":
                entity = PersonGroup.objects.create(name=f"Test Group {i}")
            elif entity_type == "gifttag":
                entity = GiftTag.objects.create(name=f"Test Tag {i}")
            else:
                continue
    ...
    @given(entity_type=st.sampled_from(["person", "gift", "event", "persongroup", "gifttag"]))
    ...
    def test_unsaved_changes_detection_property(self, entity_type):
        ...
        **Validates: Requirements 12.1, 12.2**
        ...
        assert response.status_code == 200, (
            f"Form page should be accessible for {entity_type} create"
        )
        ...
        assert 'id="main-form"' in content or "data-track-changes" in content, (
            "Form should have tracking identifier"
        )
    ...
    def test_visual_indicators_for_modified_forms_property(self, entity_type):
        ...
        **Validates: Requirements 12.3, 12.4, 12.5**
        ...
        assert "unsaved-changes.css" in content, (
            "Unsaved changes CSS should be included for visual indicators"
        )

        # Check that unsaved changes JavaScript is included (manages visual state)
        assert "unsaved-changes.js" in content, (
            "Unsaved changes JavaScript should be included for visual indicators"
        )

        # Check that forms have the proper tracking attributes or IDs for visual feedback
        assert 'id="main-form"' in content or "data-track-changes" in content, (
            "Form should have tracking identifier for visual indicators"
        )
    ...
    def test_navigation_warnings_property(self, entity_type):
        ...
        **Validates: Requirements 12.1, 12.2**
        ...
        assert "unsaved-changes.js" in content, (
            "Unsaved changes JavaScript should be included for navigation warnings"
        )

        # Check that forms have the proper tracking attributes for navigation protection
        assert 'id="main-form"' in content or "data-track-changes" in content, (
            "Form should have tracking identifier for navigation warnings"
        )
    ...
    def test_unsaved_changes_actions_property(self, entity_type):
        ...
        **Validates: Requirements 12.4, 12.5**
        ...
        save_indicators = ['type="submit"', "Save", "btn-primary", "fas fa-save"]
        ...
        cancel_indicators = ["Cancel", "btn-secondary", "data-bs-dismiss", "fas fa-times"]
