...
class BulkOperationView(LoginRequiredMixin, View):
    ...
    def handle_bulk_delete(self, request: HttpRequest, entity_type: str, entity_ids: List[str]) -> HttpResponse:
            ...
            with transaction.atomic():
                ...
                if deleted_count > 0 or shared_removed_count > 0:
                    ...
                    if permission_denied_count > 0:
                        success_message += _(", {} items skipped (insufficient permissions)").format(permission_denied_count)

                    results['message'] = success_message
                else:
                    results['success'] = False
                    if permission_denied_count > 0:
                        results['error'] = _('Insufficient permissions to delete {} items').format(permission_denied_count)
                    else:
    ...
    def handle_bulk_share(self, request: HttpRequest, entity_type: str, entity_ids: List[str]) -> HttpResponse:
